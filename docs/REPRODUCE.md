...
2026-09-16 复核完成公开 CSV 重算和独立目录的 C++ 构建。该次环境中 PyTorch 导入出现 c10.dll 初始化错误，C++ 进程启动返回 0xC0000142；因此尚未完成新环境推理复测。五图对齐和性能结果来自 2026-08-25 的实验记录，没有重新进行完整性能测量。历史绝对延迟不是其他机器应当达到的标准。
